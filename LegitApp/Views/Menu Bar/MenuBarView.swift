...
struct MenuBarView: View {
    ...
    /// Returns true if the app has been installed for at least 1 day
    private var shouldShowMenuBarBanner: Bool {
        #if DEBUG
        return true
        #else
        ...
        return Date().timeIntervalSince(firstLaunch) >= 86400 // 1 day in seconds
        #endif
    }
    ...
    private var canStartAnyService: Bool {
        runningServices.contains { $0.status == .stopped || $0.status == .unknown }
    }

    private var canStopAnyService: Bool {
        runningServices.contains { $0.status == .started || $0.status == .error }
    }

    private var hasActiveServiceTask: Bool {
        !serviceManager.activeTasks.isEmpty
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            // MARK: Banner Ad
            if let banner = caskManager.bannerConfig,
               banner.enabled,
               shouldShowMenuBarBanner,
               ...
               let linkURL = URL(string: banner.linkUrl) {
                ...
                Button {
                    ...
                    BannerImageView(imageURL: imageURL)
                        .frame(width: 300, height: CGFloat(banner.fixedHeight ?? 60))
                        .clipped()
                }
                .buttonStyle(.plain)
    ...
    private var servicesHeader: some View {
        HStack(spacing: 8) {
            ...
            Button {
                openServicesInMainWindow()
            } label: {
                Label("Open Services", systemImage: "gearshape")
                    .labelStyle(.iconOnly)
            }
            .buttonStyle(.borderless)
            .help("Open Homebrew Services")
            ...
            .buttonStyle(.borderless)
            .disabled(serviceManager.isLoading || hasActiveServiceTask || !canStartAnyService)
            .help("Start all stopped services")
            ...
            .buttonStyle(.borderless)
            .disabled(serviceManager.isLoading || hasActiveServiceTask || !canStopAnyService)
    ...
    private func openServicesInMainWindow() {
        NSApp.activate(ignoringOtherApps: true)
        NSApp.windows
            .first { !($0 is NSPanel) && $0.canBecomeMain }
            .map { $0.makeKeyAndOrderFront(nil) }
        NotificationCenter.default.post(name: .openServicesFromMenuBar, object: nil)
    }
}
