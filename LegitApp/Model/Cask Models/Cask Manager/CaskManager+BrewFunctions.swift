...
extension CaskManager {
    ...
    func install(_ cask: Cask, force: Bool = false) {
        runTask(for: cask) {
            ...
            do {
                for try await line in Shell.streamBrewCommand(["install", "--cask"] + arguments, pty: true) {
                    completeOutput += line + "\n"
            ...
            await AnalyticsManager.shared.trackCaskEvent(.caskInstallSuccess, cask: cask)
        }
    }
    ...
    func uninstall(_ cask: Cask, zap: Bool = false) {
        runTask(for: cask) {
            ...
            await AnalyticsManager.shared.trackCaskEvent(.caskUninstallSuccess, cask: cask)
        }
    }
    ...
    func update(_ cask: Cask) {
        runTask(for: cask) {
            ...
            await AnalyticsManager.shared.trackCaskEvent(.caskUpdateSuccess, cask: cask)
        }
    }
    ...
    func reinstall(_ cask: Cask) {
        runTask(for: cask) {
            ...
            await AnalyticsManager.shared.trackCaskEvent(.caskReinstallSuccess, cask: cask)
        }
    }
